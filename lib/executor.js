...
module.exports = async function execute(app, instructions, callback) {
  ...
  await setupModels(app, instructions);
  await setupMiddleware(app, instructions);
  await setupComponents(app, instructions);
...
async function setupModels(app, instructions) {
  await defineMixins(app, instructions);
  await defineModels(app, instructions);
...
async function defineMixins(app, instructions) {
  ...
  for (obj of mixins) {
    var mixin = await requireNodeOrEsModule(obj.sourceFile);
...
async function defineModels(app, instructions) {
  var registry = app.registry || app.loopback;
  for (data of instructions.models) {
    var name = data.name;
    var model;
    ...
    if (!data.definition) {
      ...
      if (data.sourceFile) {
        ...
        var code = await requireNodeOrEsModule(data.sourceFile);
...
async function runScripts(app, list, callback) {
  list = list || [];
  var functions = [];
  for (filepath of list) {
    debug('Requiring script %s', filepath);
    try {
      var bootFn = await requireNodeOrEsModule(filepath);
...
async function setupMiddleware(app, instructions) {
  ...
  for (data of middleware) {
    debug('Configuring middleware %j%s', data.sourceFile,
        data.fragment ? ('#' + data.fragment) : '');
    var factory = await requireNodeOrEsModule(data.sourceFile);
    if (data.fragment) {
...
async function setupComponents(app, instructions) {
  for (data of instructions.components) {
    debug('Configuring component %j', data.sourceFile);
    var configFn = await requireNodeOrEsModule(data.sourceFile);
    var opts = {
      useEnvVars: true,
    };
    data.config = getUpdatedConfigObject(app, data.config, opts);
