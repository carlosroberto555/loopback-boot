...
module.exports = async function compile(options) {
  ...
  var middlewareInstructions =
    await buildMiddlewareInstructions(middlewareRootDir, middlewareConfig);
  ...
  var mixinInstructions =  await buildAllMixinInstructions(
...
async function buildMiddlewareInstructions(rootDir, config) {
  var phasesNames = Object.keys(config);
  var middlewareList = [];

  for (phase of phasesNames) {
    const phaseConfig = config[phase];
    const phaseConfigKeys = Object.keys(phaseConfig);

    for (middleware of phaseConfigKeys) {
      var allConfigs = phaseConfig[middleware];
      if (!Array.isArray(allConfigs))
        allConfigs = [allConfigs];

      for (configItem of allConfigs) {
        var resolved = await resolveMiddlewarePath(rootDir, middleware, configItem);
        ...
        var middlewareConfig = cloneDeep(configItem);
...
async function resolveMiddlewarePath(rootDir, middleware, config) {
  var resolved = {
  ...
  // function
  var m = await requireNodeOrEsModule(sourceFile);
...
async function buildAllMixinInstructions(appRootDir, options, mixinSources,
  ...
  var instructionsFromMixins = await loadMixins(sourceFiles, options.normalization);
  ...
  var instructionsFromMixinDirs = await loadMixins(sourceFiles,
  ...
  sourceFiles = findMixinDefinitions(appRootDir, mixinSources,
                                     scriptExtensions);
  if (sourceFiles === undefined) return;
  var instructionsFromMixinSources = await loadMixins(sourceFiles,
                                                options.normalization);
...
async function loadMixins(sourceFiles, normalization) {
  var mixinInstructions = {};

  for(filePath of sourceFiles) {
    ...
    if (utils.fileExistsSync(metafile)) {
      ...
      _.extend(meta, await requireNodeOrEsModule(metafile));
    }
